...
for train_index, test_index in skf.split(X, y):
    ...
    # Calculate AUC scores for each classifier
    train_auc_scores = [roc_auc_score(y_train, clf.predict(X_train)) for clf in clf_list]

    # Find the indices of the worst two models
    worst_indices = np.argsort(train_auc_scores)[:2]

    # Remove the worst two models from the list
    clf_list = [clf_list[i] for i in range(len(clf_list)) if i not in worst_indices]

    # Make predictions using the remaining models
    predictions = [clf.predict(X_test) for clf in clf_list]

    # Perform majority voting
    y_pred = np.mean(predictions, axis=0)
    y_pred_majority = np.where(y_pred >= 0.5, 1, 0)
