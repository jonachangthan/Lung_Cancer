...
for i in range(10):
    ...
    skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=num)
    for train_index, test_index in skf.split(X, y):
        ...
        clf_list = []
        for i in range(5):
            inner_X_train_p = X_train_p.iloc[i * 27:(i + 1) * 27, :]

            inner_X_train = pd.concat([inner_X_train_p, X_train_n], axis=0)

            inner_y_train = np.concatenate((np.ones(27), np.zeros(27)), axis=0)
