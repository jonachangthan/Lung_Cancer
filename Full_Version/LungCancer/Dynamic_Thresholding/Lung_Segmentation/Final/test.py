...
def convert_from_dicom_to_tif(image_dcm, save_path, save16_path):
    for i in os.listdir(image_dcm):
        ...
        dicom_information = pydicom.read_file(dicom_image_path)
        ...
        #? print(str(dicom_information.InstanceNumber))
        cv2.imwrite(save_path + str(dicom_information.InstanceNumber).zfill(4) + '.tif', stack_image)

        #* uint16
        image16_array = cv2.normalize(image_array, None, 0, 65535, cv2.NORM_MINMAX)
        cv2.imwrite(save16_path + str(dicom_information.InstanceNumber).zfill(4) + '.tif', image16_array.astype('uint16'))
...
def superimpose(original_path, mask_path, save_path):
    for i in os.listdir(original_path):
        ...
        cv2.imwrite(save_path + i, final_image)
...
def overlapping(original, segmentation, overlapping_path):
    for i in os.listdir(original):
        ...
        cv2.imwrite(overlapping_path + i[:-4].zfill(4) + '.png', overlapping_image)
        cv2.imwrite("C:/VS_Code/web2/public/onlineSegementation/segementation.png", overlapping_image)

def tif_to_png_original(file_path, save_path):
    for file in os.listdir(file_path):
        image_path = os.path.join(file_path, file)
        image = cv2.imread(image_path)
        cv2.imwrite(save_path + file[:-4].zfill(4) + '.png', image)
...
model_path = "C:/Users/user/Desktop/Lung_Segmentation/modify.h5"
base_path = 'C:/Users/user/Desktop/Lung_Segmentation/'

resunet_model_path = 'C:/Users/user/Desktop/Lung_Segmentation/resunet.h5'
...
for i in range(0, 200):
    ...
    #? print(dicom_path)
    print(patient_id)
    file_path = base_path + "result/" + str(patient_id) + '/'
