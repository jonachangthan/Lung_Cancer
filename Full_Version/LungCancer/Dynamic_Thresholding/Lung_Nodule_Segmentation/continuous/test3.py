...
def continuous_nodule(original_path, mask_path, nodule_path, patient_hu, start, end, stride, nodule_information, model_path, model, backbone_shapes, nodule_start_area):
    ...
    for image_index in range(start, end, stride):
        ...
        #* Step 1 : MaskRCNN
        first_mask = maskRCNN_main(image_path, nodule_path, nodule_information, model, backbone_shapes)

        #* If Step 1 True
        if(np.sum(first_mask) != 0):
            second_mask = label_nodule(original_path, mask_path, nodule_path, patient_hu[image_index - 1], nodule_information, model_path)
            if(np.sum(second_mask) == 0):
...
for patient_id in range(1, 44):
    ...
    nodule_path = "C:/Users/user/Desktop/Lung_Nodule_Segmentation/result3/" + str(patient_id) + "/"
