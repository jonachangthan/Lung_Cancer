...
df = pd.DataFrame(columns=["patient_id", "nodule_no", "image_no", "next_x", "next_y"])
...
def continuous_nodule(original_path, mask_path, nodule_path, patient_hu, start, end, stride, nodule_information, model_path, model, backbone_shapes, nodule_start_area):
    ...
    patient_id = int(original_path.split("/")[-3])
    nodule_no = nodule_information[1]
    ...
    for image_index in range(start, end, stride):
        ...
        df.loc[len(df.index)] = [patient_id, nodule_no, image_index, next_x, next_y]
        ...
        #* Method 2 : MaskRCNN
        second_mask = maskRCNN_main(image_path, nodule_path, nodule_information, model, backbone_shapes)
        
        #* 判斷終止條件
        if abs(np.sum(nodule_area / 255) - np.sum(first_mask / 255)) < abs(np.sum(nodule_area / 255) - np.sum(second_mask / 255)):
            final_mask = first_mask
        else:
            final_mask = second_mask
...
def main(dicom_path, image_path, mask_path, nodule_path, nodule_information, model_path, model, backbone_shapes):
    image_no = nodule_information[0]
    nodule_no = nodule_information[1]
    coordinate_x = nodule_information[2]
    coordinate_y = nodule_information[3]
    ...
    df.loc[len(df.index)] = [patient_id, nodule_no, image_no, coordinate_x, coordinate_y]
...
nodule_csv_path = "D:/Hospital_data/nodule/nodules_final.csv"
answer_path = "D:/Hospital_data/nodule/total/mask/"
...
def find_coordinate(patient_index, nodule_path):
    ...
    for i in num:
        ...
        num_subset = num_subset.sample(n=1).reindex(columns=['filename', 'num', 'cordX', 'cordY']).values.flatten()
        num_subset[0] += 1
...
for patient_id in range(1, 44):
    if not os.path.isdir(answer_path + str(patient_id).zfill(3)):
        continue
    print(patient_id)
    dicom_path = "D:/Hospital_data/dicom/" + str(patient_id) + "/"
    dicom_path = dicom_path + os.listdir(dicom_path)[0] + "/"
    print(dicom_path)

    image_path = "C:/Users/user/Desktop/Lung_Segmentation/result/" + str(patient_id) + "/original16/"
    mask_path = "C:/Users/user/Desktop/Lung_Segmentation/result/" + str(patient_id) + "/segmentation2/"
    nodule_path = "C:/Users/user/Desktop/Lung_Nodule_Segmentation/result1_2/" + str(patient_id) + "/"
    if not os.path.isdir(nodule_path): os.mkdir(nodule_path)

    patient_nodule_information = find_coordinate(patient_id, nodule_csv_path)
    for j in range(len(patient_nodule_information)):
        nodule_information = patient_nodule_information[j] # image_no, nodule_no, coordinate_x, coordinate_y
        main(dicom_path, image_path, mask_path, nodule_path, nodule_information, model_path, model, backbone_shapes)

with pd.ExcelWriter(engine='openpyxl', path='C:/Users/user/Desktop/Lung_Nodule_Segmentation/result.xlsx', mode='a') as writer:
    df.to_excel(writer, sheet_name='1_2', index=False)