...
async function fetchBtnColor() {
    const requestURL = '/mask_btn_color.json';
    const request = new Request(requestURL);
    const response = await fetch(request);
    const superPatients = await response.json();
    populateBtnColor(superPatients);
}
function populateBtnColor(obj) {
...
for (var i = 0; i < buttonList.length; i++) { //監聽所有btn click後 填上value(病人編號)發出form
