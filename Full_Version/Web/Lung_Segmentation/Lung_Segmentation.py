...
def convert_from_dicom_to_tif(image_dcm, save_path, save16_path):
    for i in os.listdir(image_dcm):
        ...
        cv2.imwrite(save_path + '0001.tif', stack_image)

        #* uint16
        image16_array = cv2.normalize(image_array, None, 0, 65535, cv2.NORM_MINMAX)
        cv2.imwrite(save16_path + '0001.tif', image16_array.astype('uint16'))
...
def superimpose(original_path, mask_path, save_path):
    for i in os.listdir(original_path):
        ...
        cv2.imwrite(save_path + '0001.tif', final_image)
...
def final_lung_segmentation(image_path, mask_path, model_path):
    ...
    for i, pred in enumerate(preds):
        ...
        image = (image * 255).astype('uint8')
        cv2.imwrite(mask_path + str(i + 1).zfill(4) + ".tif", image)
        cv2.imwrite("C:/VS_Code/web2/public/onlineSegementation/lungMask.png", image)
...
dicom_path = 'C:/VS_Code/web2/segementationdcm/'
