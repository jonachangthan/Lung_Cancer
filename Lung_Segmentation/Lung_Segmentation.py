...
def convert_from_dicom_to_tif(image_dcm, save_path, save16_path):
    for i in os.listdir(image_dcm):
        ...
        cv2.imwrite(save_path + i[:-4].split('-')[-1].zfill(4) + '.tif', stack_image)

        #* uint16
        image16_array = cv2.normalize(image_array, None, 0, 65535, cv2.NORM_MINMAX)
        cv2.imwrite(save16_path + i[:-4].split('-')[-1].zfill(4) + '.tif', image16_array.astype('uint16'))
...
def tif_to_png_segmentation(file_path, save_path):
    for file in os.listdir(file_path):
        ...
        cv2.imwrite(save_path + file[:-4].zfill(4) + '.png', image_8)
...
model_path = "E:/Lung_Cancer/System/Version1/Lung_Segmentation/modify.h5"
base_path = "E:/VS_Code/LungCancer"
dicom_path = "F:/LungCancer/1"
resunet_model_path = "E:/Lung_Cancer/System/Version1/Lung_Segmentation/resunet.h5"
